...
function getBrands(){
  ...
  userTasksRef.get()
    ...
    .catch((error) => {
      alert('Errore durante il recupero dei marchi: ', error);
    });
...
function getTasks(){
  var table1 = document.getElementById("tasksTableBody");
  ...
  userTasksRef.get()
    .then((querySnapshot) => {
      querySnapshot.forEach((doc) => {
        const data = doc.data();
        createTasksTableRow(data);
      });
    })
    .catch((error) => {
      alert('Errore durante il recupero dei Task: ', error);
    });
